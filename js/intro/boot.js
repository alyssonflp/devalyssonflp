...
window.addEventListener("load", () => setTimeout(typePassword, 400));
...
function typePassword() {
  ...
  const interval = setInterval(()=>{
    input.value += pass[i++];
    if(i>=pass.length){ clearInterval(interval); startProgress(); }
  }, 100);
}
...
function startProgress() {
  ...
  const timer = setInterval(()=>{
    ...
    if(progress>=100){ clearInterval(timer); showDesktop(); }
  }, 30);
}
...
function showDesktop() {
  ...
  setTimeout(()=>{ login.style.display="none"; desktop.classList.remove("hidden"); window.startOS?.(); },600);
}
